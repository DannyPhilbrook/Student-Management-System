...
namespace Testing_Project
{
    public partial class NewClass : UserControl
    {
        ...
        {
            string dbPath = @"Data Source=Database/stdmngsys.db;Version=3;";
            ...
            try
            {
                ...
                // Optionally: add a placeholder if no labels exist
                if (cmbCourseLabel.Items.Count == 0)
                {
                    cmbCourseLabel.Items.Add("(No labels found)");
                    cmbCourseLabel.SelectedIndex = 0;
                    cmbCourseLabel.Enabled = false;
                }
                else
                {
                    cmbCourseLabel.Enabled = true;
                }
            }
            catch (Exception ex)
            {
